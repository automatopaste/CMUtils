package cmu.subsystems;

import cmu.plugins.SubsystemCombatManager;
import cmu.subsystems.ai.BaseSubsystemAI;
...
public abstract class BaseSubsystem implements Subsystem, BaseSubsystemAI {
    ...
    public BaseSubsystem(String systemId) {
        this.data = SpecLoadingUtils.getSubsystemData(systemId);
        if (data == null) throw new NullPointerException("Subsystem data is null: " + systemId);
    }
    public BaseSubsystem(SubsystemData data) {
        this.data = data;
        if (data == null) throw new NullPointerException("Subsystem data is null");
    }
    public BaseSubsystem() {
        String id = getSubsystemId();
        this.data = SpecLoadingUtils.getSubsystemData(id);
    }

    @Override
    public void init(ShipAPI ship) {
        this.ship = ship;
        subsystemId = "Subsystem_" + this.hashCode() + "_" + ship.hashCode();
    ...
    public Vector2f guiRender(Vector2f inputLoc, Vector2f rootLoc) {
        ...
        return CombatUI.drawSubsystemStatus(
                ship,
                guiLevel,
                getName(),
                info,
                ...
                getActiveHotkey(),
                flavour,
                SubsystemCombatManager.showInfoText,
                getNumGuiBars(),
                inputLoc,
                rootLoc
        );
